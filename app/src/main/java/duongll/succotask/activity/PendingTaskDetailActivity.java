...
public class PendingTaskDetailActivity extends AppCompatActivity {
        ...
        callTask.enqueue(new Callback<Task>() {
            ...
            public void onResponse(Call<Task> call, Response<Task> response) {
                if (response.code() == 200) {
                    Toast.makeText(PendingTaskDetailActivity.this, "Decline task successfully", Toast.LENGTH_SHORT).show();
                    return;
                } else {
                    Toast.makeText(PendingTaskDetailActivity.this, "Decline task failed", Toast.LENGTH_SHORT).show();
                    return;
                }
            }
            ...
            public void onFailure(Call<Task> call, Throwable t) {
                Toast.makeText(PendingTaskDetailActivity.this, "Have error when connect to backend", Toast.LENGTH_SHORT).show();
                return;
            }
        });
    }
    ...
    private void clickToAcceptPendingTask(){
        ...
        callTask.enqueue(new Callback<Task>() {
            ...
            public void onResponse(Call<Task> call, Response<Task> response) {
                if (response.code() == 200) {
                    Toast.makeText(PendingTaskDetailActivity.this, "Accept pending task successfully", Toast.LENGTH_SHORT).show();
                    return;
                } else {
                    Toast.makeText(PendingTaskDetailActivity.this, "Accept pending task failed", Toast.LENGTH_SHORT).show();
                    return;
                }
            }
